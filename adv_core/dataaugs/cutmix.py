...
def cutmix(images, labels, alpha=1.0, beta=1.0, num_classes=10, cut_window=0):
    """
    ...
    Arguments:
        ...
        cut_window: fix the window size instead sampled from Beta Distribution
    Returns:
    ...
    if cut_window > 0:
#         print("  ### cut_window {} ### ".format(cut_window))
        # In Fixing Data Augmentation paper, they studied a fixed window-size instead of sample from a distribution.
        cut_w, cut_h = np.array(cut_window, dtype=np.int32), np.array(cut_window, dtype=np.int32)
    else:
